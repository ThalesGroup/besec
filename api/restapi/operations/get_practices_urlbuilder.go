...
func (o *GetPracticesURL) Build() (*url.URL, error) {
	...
	var _path = "/practices/{version}"

	version := o.Version
	if version != "" {
		_path = strings.Replace(_path, "{version}", version, -1)
	} else {
		return nil, errors.New("version is required on GetPracticesURL")
	}
...
func (o *GetPracticesURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetPracticesURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetPracticesURL")
	}
